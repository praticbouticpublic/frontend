...
export class LogininfoService {
  ...
  setIdentifiant(lidentifiant: string)
  {
  ...
  async setMotdepasse(lemotdepasse: string) {
    ...
    const iv = crypto.getRandomValues(new Uint8Array(16)); // Générer un nouveau IV
  ...
  async getMotdepasse(useStorage: boolean = true): Promise<string> {
    ...
    const iv = this.base64ToUint8Array(ivBase64); // Convertir le IV de base64 en Uint8Array
  
    try {
        const key = await crypto.subtle.importKey('raw', rawKey, { name: 'AES-CBC' }, true, ['decrypt']);
        const encryptedData = this.base64ToUint8Array(encryptedVal); // Convertir le texte chiffré en Uint8Array
